...
{
    public class WelcomeSymbols : StoryboardObjectGenerator
    {
        [Configurable]
        ...
        public void generatePerCharacter(FontGenerator font, SubtitleSet subtitles, StoryboardLayer layer, bool additive)
        {
            foreach (var subtitleLine in subtitles.Lines)
            {
                ...
                foreach (var line in subtitleLine.Text.Split('\n'))
                {
                    ...
                    var letterX = 170 - lineWidth * 0.5f;
                    foreach (var letter in line)
                    {
                        var texture = font.GetTexture(letter.ToString());
                        if (!texture.IsEmpty)
                        {
                            ...
                            sprite.MoveX(subtitleLine.StartTime+1, 1699, position.X, position.X/1.1 +30);
                            sprite.MoveX(1699, 1886, position.X/2 +155, position.X/2.1+163);
                            sprite.Scale(1699, 1886, FontScale*2, FontScale*2);
                            sprite.MoveX(1886, 2074, position.X/1.3+72, position.X/1.2+52);
                            sprite.Scale(1886, 2074, FontScale*4, FontScale*4.1);
                        }
                        letterX += texture.BaseWidth * FontScale+100;
                    }
                    letterY += lineHeight;
                }
            }
